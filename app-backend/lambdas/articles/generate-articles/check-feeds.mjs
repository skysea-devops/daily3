...
async function loadSources() {
  ...
  // Age windows come from the Lambda too — a local copy would silently drift
  // and report feeds as STALE against limits that no longer exist.
  let articleMaxAge = {};
  try {
    articleMaxAge = evalLiteral(
      extractObjectLiteral(src, "ARTICLE_MAX_AGE_DAYS: Record<string, number> ="),
    );
  } catch {
    console.warn("! Could not read ARTICLE_MAX_AGE_DAYS; falling back to 30 days for every category.");
  }
  const podcastMatch = src.match(/PODCAST_MAX_AGE_DAYS\s*=\s*(\d+)/);
  const podcastMaxAge = podcastMatch ? Number(podcastMatch[1]) : 45;

  return { sourceFile, articles, podcasts, articleMaxAge, podcastMaxAge };
}
...
async function checkFeed(kind, category, source, windows) {
  const r = {
    ...
    status: null, items: 0, newestAgeDays: null, state: "OK", notes: [],
  };
  ...
  if (res.error) {
    ...
    r.state = "BLOCKED";
    r.notes.push(`network error after ${RETRIES + 1} attempts: ${res.error} — rerun before acting on this`);
    return r;
  }
  if (!res.ok) {
...
async function main() {
  const { sourceFile, articles, podcasts, articleMaxAge, podcastMaxAge } = await loadSources();
